...
class CoordinateSystemTest(QWidget):
    ...
    def analyze_coordinate_systems(self):
        ...
        for i, screen in enumerate(screens):
            ...
            analysis += f"  Coordinate System Analysis:\n"
            if geometry.x() < 0:
                analysis += f"    - NEGATIVE X coordinate: Screen is positioned LEFT of primary\n"
            elif geometry.x() > 0:
                analysis += f"    - POSITIVE X coordinate: Screen is positioned RIGHT of primary\n"
            else:
                analysis += f"    - ZERO X coordinate: Screen aligns with primary horizontally\n"
            
            if geometry.y() < 0:
                analysis += f"    - NEGATIVE Y coordinate: Screen is positioned ABOVE primary\n"
            elif geometry.y() > 0:
                analysis += f"    - POSITIVE Y coordinate: Screen is positioned BELOW primary\n"
            else:
                analysis += f"    - ZERO Y coordinate: Screen aligns with primary vertically\n"
            
            # Corner coordinates
            analysis += f"  Corner Coordinates:\n"
        ...
        if current_screen:
            window_pos = self.pos()
            analysis += f"Current Window Information:\n"
    ...
    def test_window_positioning(self):
        ...
        for i, screen in enumerate(screens):
            ...
            if (center_x >= geometry.x() and 
                center_x + self.width() <= geometry.x() + geometry.width() and
                center_y >= geometry.y() and 
                center_y + self.height() <= geometry.y() + geometry.height()):
                self.analysis_text.append(f"  ✓ Valid positioning coordinates")
            else:
                self.analysis_text.append(f"  ✗ Invalid positioning coordinates")
